...
def scan_index(obj, df_scrip, index_key: str, vix_ltp: float,
               risk_manager, paper_trader, current_positions: dict) -> dict:
    """
    Run full analysis pipeline for one index.
    Returns result dict with all data + decision.

    current_positions: {index_key: position_dict or None}
    """
    ...
    logger.info(f"✅ {index_key} scan done → {decision.get('action')} ({regime['regime']})")
    return result


def _send_index_summary(index_key, spot_ltp, summary,
                        greeks, regime, confluence, decision, expiry):
